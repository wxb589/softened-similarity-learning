...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, pretrained=True, cut_at_pooling=False,
        ...
        if self.FCN:
            ...
            self.local_mask = nn.Conv2d(self.reduce_dim, 8, kernel_size=1, padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        if self.FCN:
            ...
            f6 = x * (lw[6].expand_as(x))
            f7 = x * (lw[7].expand_as(x))
            ...
            f6 = F.avg_pool2d(f6, kernel_size=(f6.size(2), f6.size(3)))
            f7 = F.avg_pool2d(f7, kernel_size=(f7.size(2), f7.size(3)))
            x = torch.cat((f0, f1, f2, f3, f4, f5, f6, f7), 2)
            feat = torch.cat((f0, f1, f2, f3, f4, f5, f6, f7), 2)
            ...
            print(out1.shape)
            parts = torch.cat((x0, x1, x2, x3, x4, x5, x6, x7))
            # parts = parts.view(parts.size(1), parts.size(0), -1)
